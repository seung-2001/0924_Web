...
public class ElServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * 1. application scope : ServletContext 한 애플리케이션 당, 단 한 개 존재하는 객체 이 영역에 데이터를
		 * 담으면 애플리케이션 전역에서 사용 가능
		 * 
		 * 2. session scope : HttpSession 하나의 부라우저 당, 한 개 존재하는 객체 이 영역에 데이터를 담으면
		 * JSP/Servlet단에서 사용 가능 값이 한 번 담기면 서버를 멈추거나, 세션을 비우거나, 브라우저가 닫히기 전까지는 사용 가능
		 * 
		 * 3. request scope : HttpServletRequest 요청 및 응답 시 매 번 생성되는 객체 이 영역에 데이터를 담으면 해당
		 * request객체를 포워딩 받는 응답 JSP에서만 사용가능(1회성)
		 * 
		 * 4. page scope : PageContext JSP페이지 내에서만 사용 가능
		 * 
		 * => 위 객체들에 속성을 추가할 때는 .setAttribute("키", 벨류); .getAttribute("키");
		 * .removeAttribute("키");
		 * 
		 */
