...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * 
		 * - request : 서버로 요청 보낼 때의 정보(요청 시 전달값, 요청 전송방식, 사용자정보) - response : 요청에 대한
		 * 응답데이터를 만들 때 사용
		 */
		// 절차
		...
		// 성공했을 경우 : 조회성공한 컬럼값을 필드에 담은 맴버객체 주소값
		// 실패했을 경우 : null값
		...
		// 1. 로그인 된 사용자의 정보를 어딘가에 담을 것(application, session,
		// request, page)

		/*
		 * 크다 1) application : 웹 애플리케이션 전역에서 사용 가능 (일반 자바 클래스에서 값을 사용할 수 있음) 2) session
		 * : 모든 JSP와 Servlet에서 꺼내 쓸 수 있음 단, session에 값이 지워지기 전까지 세션 종료 시점 : 브라우저 종료, 서버
		 * 멈춤, 코드로 지움
		 * 
		 * 3) request : 해당 request를 포워딩한 응답 JSP에서만 쓸 수 있음 요청 부터 응답까지만 딱 쓸 수 있음
		 * 
		 * 4) page : 값을 담은 JSP에서만 쓸 수 있음 작다
		 * 
		...
		if (loginMember != null) {
			...
			// response객체를 이용
			// respose.sendRedirect("/다시 요청 보낼 URL경로");
			// welcomefiel => contextRoot
